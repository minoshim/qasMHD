...
bxct=np.zeros((nt,ny,nx+1),dtype=np.float32)
byct=np.zeros((nt,ny+1,nx),dtype=np.float32)
sst=0
for sst in range(0,nt):
    tmp=np.fromfile(direc+f"merge_outdat_{sst:05d}.dat",dtype=np.float32).reshape((nd,ny,nx))
    data[sst,:,:,:]=tmp
    bxct[sst],byct[sst]=read_ct_fields(direc,sst,nx,ny)
...
# jz.shape=(nt,ny-1,nx-1); xjz,yjz are its coordinates.
xjz=0.5*(x[1:]+x[:-1])
yjz=0.5*(y[1:]+y[:-1])
