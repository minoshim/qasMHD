...
try
{
  ...
  double vx_1=0.0;	// Velocity in X
  double vy_1=-1.0;	// Velocity in Y
  double vz_1=0.0;	// Velocity in Z
  double ma_u=100.0;	// Sound Mach number
  double pr_1=(ro_1*vy_1*vy_1)/(gam*ma_u*ma_u); // Pressure
  double beta=1e5;				// Beta
  double b0_1=(MAGNET)*sqrt(2.0*pr_1/beta);	// Magnetic field
  double bx_1=b0_1;
  double by_1=0.0;
  double bz_1=0.0;
  double vref=-0.6;	// Velocity of reference. 0 = shock rest frame
  double dw=0.5*dy;	// Shock width
  //// Contact Discon parameters
  double ro_3=ro_1*1e1;	// Density jump
  double dro3=RANDOM*ro_3*0.1;	// Density perturbation (Available when RANDOM=1)
  ...
  // Store boundary parameters per instance, with velocity in the simulation frame.
  inflow={ro_3,dro3,vx_1,vy_1-vref,vz_1,pr_1,bx_1,by_1,bz_1};
  ...
  // Density perturbations use a separate stream for each MPI rank.
  std::seed_seq seeds{seed,static_cast<unsigned>(mpi_rank)};
  noise_engine.seed(seeds);
  std::vector<double> ypos;
  resize_work(ypos,nx); // Allocation failure must stop all ranks.
  for (i=0;i<nx;i++){
  ...
  for (int m=2;m<=mmax;m++){
    double xphase;
    if (mpi_rank == 0) xphase=rand_noise_mt(para,noise_engine);
    mpi2d_io::check_mpi(MPI_Bcast(&xphase,1,MPI_DOUBLE,0,MPI_COMM_WORLD),
                        "Random phase broadcast failed.");
    for (i=0;i<nx;i++){
  ...
  mpi2d_io::abort_run("Exception while initializing RMI state or random generator.");
}
