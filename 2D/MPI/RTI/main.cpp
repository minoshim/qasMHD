...
int main(int argc, char* argv[])
{
  ...
  while(n++ < nmax && tim < tend){
    ...
    // Boundary condition
    boundary(p,8,nx,ny,xoff,yoff,stxs,dnxs,stys,dnys,gam,phi_g,mpi_rank,mpi_numx,mpi_numy);

    // MHD update
    mhd_fd2d(p,dt,dx,dy,8,nx,ny,xoff,yoff,gam,phi_g,mpi_rank,mpi_numx,mpi_numy);
