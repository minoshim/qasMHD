...
/* Include modification for electron MHD. de is electron inertia length */
{
  int i,ss,rk;
  ...
  /* Modification for electron MHD */
  double *qy,*qz;
  qy=(double*)malloc(sizeof(double)*nx);
  qz=(double*)malloc(sizeof(double)*nx);
  emhd_b2q(by,qy,de,dx,nx,xoff,dnxs[4]); /* Boundary condition included */
  emhd_b2q(bz,qz,de,dx,nx,xoff,dnxs[5]); /* Boundary condition included */
  p[4]=qy;
  p[5]=qz;
...
#pragma simd
	...
	ss=i;
	double *val[]={&p[0][ss],&p[1][ss],&p[2][ss],&p[3][ss],&p[4][ss],&p[5][ss],&p[6][ss]};
    ...
    /* Modification for electron MHD */
    emhd_q2b(qy,by,de,dx,nx,xoff,dnxs[4]); /* Boundary condition included */
    emhd_q2b(qz,bz,de,dx,nx,xoff,dnxs[5]); /* Boundary condition included */
  }
