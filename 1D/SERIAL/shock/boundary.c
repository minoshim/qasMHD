...
/* [istt,iend]=[1,1] => set right boundary only */
{
  ...
  for (i=istt;i<=iend;i++){		/* Left @ i=0, Right @ i=1 */
    ...
    /* Primitive variables inside the boundary */
    {
      ...
      byb[j]=by[ss];
      bzb[j]=bz[ss];
      mhd_prmtv(rob[j],mx[ss],my[ss],mz[ss],bxb[j],byb[j],bzb[j],en[ss],
		&vxb[j],&vyb[j],&vzb[j],&prb[j],gamma);
    }
    ...
    /* Set boundary condition */
    {
      ...
      mhd_cnsvt(ro[ss],mx[ss],my[ss],mz[ss],bx,by[ss],bz[ss],en[ss],
		&mx[ss],&my[ss],&mz[ss],&en[ss],gamma);
    }
