...
int main(int argc, char* argv[])
{
  ...
  while(n++ < nmax && tim < tend){
    ...
    // Boundary condition
    boundary(p,8,nx,ny,nz,xoff,yoff,zoff,stxs,dnxs,stys,dnys,stzs,dnzs,gam,phi_g,mpi_rank,mpi_numx,mpi_numy,mpi_numz);

    // MHD update
    mhd_fd3d(p,dt,dx,dy,dz,8,nx,ny,nz,xoff,yoff,zoff,gam,phi_g,mpi_rank,mpi_numx,mpi_numy,mpi_numz);
